...
export function DocumentTypesClient({ documentTypes }: DocumentTypesClientProps) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = editing
        ? await updateDocumentType(editing.id, formData)
            ...
            <DropdownMenuItem
              onClick={() =>
                startTransition(async () => {
                  ...
                  const result = await toggleDocumentTypeStatus(
                    row.original.id,
                    newStatus
                  );
                  if (result.error) toast.error(result.error);
                  else toast.success(`Document type ${newStatus}`);
                })
              }
            >
    ...
    <>
      <div className="mb-4 flex justify-end">
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {editing ? "Edit Document Type" : "Create Document Type"}
              </DialogTitle>
            </DialogHeader>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (!deleteId) return;
          startTransition(async () => {
            const result = await deleteDocumentType(deleteId);
            if (result.error) toast.error(result.error);
